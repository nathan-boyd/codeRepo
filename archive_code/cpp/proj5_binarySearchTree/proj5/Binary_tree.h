...
template <class Entry>
void Binary_tree<Entry>::recursive_copy(Binary_node<Entry> *original, Binary_node<Entry> *& newTreePtr) const
{
  if (original != NULL) {
    newTreePtr = new Binary_node<Entry>(original->data);
    recursive_copy(original->left, newTreePtr->left);
    recursive_copy(original->right, newTreePtr->right);
  }
  else
    newTreePtr = NULL;
}
...
template <class Entry>
void Binary_tree<Entry>::recursive_insert(const Entry &key, Binary_node<Entry>* &leaf) const
{
    if(leaf == NULL)
    {
        Binary_node<Entry> *newNode = new Binary_node<Entry>(key);
        leaf = newNode;

    }
    else
    {
        int left = recursive_height(leaf->left);
        int right = recursive_height(leaf->right);

        if(left <= right)
            recursive_insert(key, leaf->left);
        else
            recursive_insert(key, leaf->right);
